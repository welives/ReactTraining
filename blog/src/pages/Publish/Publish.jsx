...
export default function Publish() {
  ...
  const [curCateKey, setCurCateKey] = useState('') // 当前分类的key 因为文章表结构只有分类id字段,所以需要一个中间变量进行等效转换
  // 请求分类列表
  useEffect(() => {
    ;(async () => {
      ...
      if (data.status === 'success') {
        ...
        if (state) {
          setCurCateKey(
            data.result.data.find((el) => el.id === state.category_id).key
          )
        }
      }
    })()
  ...
  useEffect(() => {
    ...
    if (state) {
      setCurCateKey(cates.find((el) => el.id === state.category_id)?.key)
    } else {
      setCurCateKey('')
    }
  }, [state])
  ...
  const handleCategoryChange = (e) => {
    ...
    setForm({ ...form, category_id: cateId })
  }
  /**
   * 表单提交事件
   * @param {SubmitEvent} e
  ...
  const handleFileChange = (e) => {
    ...
    if (e.file.status === 'done') {
      setForm({ ...form, cover: e.file.response.result.data })
